...
SELECT COUNT(*) AS total_rentals, CONCAT(customer.first_name, ' ', customer.last_name)
FROM customer
...
SELECT SUM(payment.amount) AS dollars_spent, CONCAT(customer.first_name, ' ', customer.last_name)
FROM customer
...
SELECT store.store_id, address.address, COUNT(rental.rental_id) AS total_rentals, SUM(payment.amount) AS total_sales, AVG(payment.amount) AS avg_sale
FROM store
...
SELECT CONCAT(actor.first_name, ' ', actor.last_name) AS full_name, COUNT(*) AS number_of_rentals
FROM actor
JOIN film_actor ON actor.actor_id = film_actor.actor_id
JOIN film ON film_actor.film_id = film.film_id
JOIN inventory ON film.film_id = inventory.film_id
...
SELECT CONCAT(actor.first_name, ' ', actor.last_name) AS full_name, COUNT(*) AS number_of_comedy_rentals
FROM actor
JOIN film_actor ON actor.actor_id = film_actor.actor_id
JOIN film ON film_actor.film_id = film.film_id
JOIN film_category ON film.film_id = film_category.film_id
...
WHERE category.name = 'Comedy'
GROUP BY full_name
ORDER BY number_of_comedy_rentals DESC LIMIT 5;